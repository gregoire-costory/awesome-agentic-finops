...
- [Zopnight](https://zop.dev/learn/mcp-server?utm_source=awesome-agentic-finops&utm_medium=listing&utm_campaign=mcp-directory) `vendor` - ZopDev MCP: cloud cost and infrastructure governance across AWS, Azure, GCP, Databricks and Snowflake. 263 tools (155 read, 108 write) covering cost, resources, schedules, recommendations, budgets, governance and diagnostics over a hosted remote MCP endpoint. Read-only by default, with optional scoped writes.
